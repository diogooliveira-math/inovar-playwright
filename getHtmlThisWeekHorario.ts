...
export async function getHtmlThisWeekHorario() {
  let browser;
  let inovar: BrowserManagement | undefined;

  try {
    log('info', 'getHtmlThisWeekHorario starting.');
    ...
    log('info', `getHtmlThisWeekHorario finished successfully (${html.length} bytes).`);
    console.log(JSON.stringify(output, null, 2));

  } catch (error) {
    log('info', 'Error in getHtmlThisWeekHorario:', error);

    if (inovar) {
      log('info', 'Saving failure screenshot ...');
      await inovar.saveScreenshot('getHtmlThisWeekHorario_failure');
    }
